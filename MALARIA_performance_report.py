...
perfotemp_hiv = wb_perfotemp["MALARIA "]
...
for sheet in kit_names:
    ...
    for id_cells in lab_ids:  # cell loop for lab ids
        ...
        try:
            os.makedirs("D:\\MALARIA\\Lab_ID_" + str(int(lab_no)))
        except:
            pass
        if (run == 0):
            temp_run = 1
            ...
            temp_run = run
        wb_perfotemp.save(
            "D:\\MALARIA\\Lab_ID_" + str(int(lab_no)) + "\\MALARIA_" + str(int(lab_no)) + "_Run_" + str(int(temp_run)) + ".xlsx")
